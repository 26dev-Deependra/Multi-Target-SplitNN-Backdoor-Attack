...
parser.add_argument('--label', type=int, required=True,
                    help='the target class of your attack')
...
def train_model(model, dataloader, label, steal_set, steal_id, epoch_num, start_epoch=0, is_binary=False, verbose=True):
    ...
    for epoch in range(start_epoch, epoch_num):
        ...
        for i, (x_in, y_in, id_in) in enumerate(dataloader):
            ...
            if args.unit != 1:
                x_list = x_in.split([int(x_in.size()[2]*args.unit)]+[int(x_in.size()[2]*other_unit) for i in range(args.multies-2)]+[
...
def attack_model(model, dataloader, vec_arr, label, multies, unit, other_unit):
    ...
    for i, (x_in, y_in, _) in enumerate(dataloader):
        ...
        if args.unit != 1:
            x_list = x_in.split([int(x_in.size()[2]*unit)]+[int(x_in.size()[2]*other_unit) for i in range(multies-2)]+[
        ...
        if args.unit != 1:
            vec = torch.cat([vec1]+[model.models[i](x_list[i])
                            for i in range(1, multies)], dim=2)
        else:
            vec = vec1
...
if __name__ == '__main__':
    ...
    atkacc = attack_model(model, testloader, last_vec_arr,
